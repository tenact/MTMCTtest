    ...
    for i, box in enumerate(bbox):
       ...
       # if(addIdToJsonFile):
        ...
        if id not in data_deque:  
            ...
            Dictionary mit drei Elementen: id, cluster_id und ResNet50 Feature Array
            aber komplett falscher Ansatz, da das Hierarchical Clustering nach der Erstellung aller Local-Tracklets
            durchgeführt werden muss.

            Hier muss ich stattdessen eine andere Datenstruktur verwenden, zum Beispiel ein Tree.
            ...
            if len(features_dict) == 0:
                features_dict[1] = {'feature': feature_bild, 'cluster': 0, 'id': id}
                print(features_dict[1])
            elif len(features_dict) == 1:
                features_dict[2] = {'feature': feature_bild, 'cluster': 0, 'id': id}
                print(features_dict[2])

            elif len(features_dict) == 2:


                X = np.array([(f['id'], f['feature'].reshape(1, -1)) for f in features_dict.values()])

                #model = AgglomerativeClustering(n_clusters=10, affinity='euclidean', linkage='ward', distance_threshold=0.5)
                clustering = AgglomerativeClustering().fit(X)

                

                #model = AgglomerativeClustering(n_clusters=None, distance_threshold=0.5, linkage='ward', affinity='euclidean')
                ...
                #Check if the new feature is similar to any of the features in the same cluster
                similar = False
                similar_feature_id = None
                for feature_id, feature in features_dict.items():
                    #if feature['cluster'] == cluster_label:
                    dist = reid.euclidian_distance(features_dict[feature_id]['feature'], feature_bild)
                    if dist < 0.3:
                        print(f"New feature is similar to existing feature {feature_id}")
                        # print({feature_bild})
                        print("die Distans ist ?")
                        print("Euclidean Distance: " + str(dist))

                        similar = True
                        similar_feature_id = feature_id
                        newID = feature_id
                        id = feature_id
                        addIdToJsonFile = False
                        break
                 #If the new feature is not similar to any existing feature, add it to the dictionary and update the model
                if not similar:
                    features_dict[id] = {'feature': feature_bild, 'cluster': cluster_label, 'id' : id}
                    X = np.array([f['feature'] for f in features_dict.values()])
                    model.fit(X)
                    # Save the updated model to a pickle file
                    features_dict[id] =feature_bild

                    with open('hierarchical_model.pkl', 'wb') as f:
                        pickle.dump(model, f)
                else:
                    # Add the feature ID to the similar feature in the dictionary
                    features_dict[similar_feature_id] = {'feature': feature_bild, 'cluster': cluster_label, 'id' : id}
                ...
                cluster_labels = model.fit_predict([feature_bild] + [f['feature'] for f in features_dict.values()])
            ...
            if len(features_dict) == 0:
                ...
                for fe, arr in features_dict.items():
                    dist = reid.euclidian_distance(arr, feature_bild2)
                    print(dist)
                    if dist < 0.3:
                        neuId = fe
                        addIdToJsonFile = False
