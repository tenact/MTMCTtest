...
import sys
sys.path.append("deep-person-reid/torchreid/utils")
...
from hardFeatures import SIFTFeatures
import torchreid
import json
...
from deep_person_reid import DeepPersonReID
...
def draw_boxes(img, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        if id not in data_deque:  
            ...
            sub_image.resize(224,224)      


            #Versuch hier die Features mit der Deep Person ReID zu extrahieren: funktioniert nicht

            '''
            model = DeepPersonReID()

            model.eval()
            image_tensor = Variable(sub_image)
            features = model(image_tensor)
            '''
            
            extractor = FeatureExtractor(
            model_name='osnet_x1_0',
            model_path='shufflenet-bee1b265.pth.tar', # verwendung eines preTrained Models
            device='cuda'
            )
            features = extractor(sub_image)    
           ...
           # print(features_dict)
            ...
            if len(features_dict) == 0:
                features_dict[1] = (features, 0, id) # global ID is DEFAULT ZERO
