...
def draw_boxes(img, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        # code to find center of bottom edge, to draw trail
        center = (int((x2+x1)/ 2), int((y2+y2)/2))
        id = int(identities[i]) if identities is not None else 0

        if id not in data_deque:  
            data_deque[id] = deque(maxlen= 13)
            ...
            id_bla = len(features_dict)
           
            if len(features_dict) > 2:
                # Load the existing hierarchical clustering model
                with open('hierarchical_model.pkl', 'rb') as f:
                    model = pickle.load(f)

               # cluster_label = cluster_labels[0]
                print(" die länge der Liste" + str(len(features_dict)))
                #print("cluster_label: " + str(cluster_label))


                if(len(features_dict) == 100):
                ...
                for feature_id, feature in features_dict.items():
                   # if feature['cluster'] == cluster_label:
                    dist = reid.euclidian_distance(feature['feature'], feature_bild)
                    if dist < 0.3:
                ...
                if not similar:
                    print("bilder sind nicht gleich")
                    features_dict[id_bla] = {'feature': feature_bild, 'cluster': cluster_label, 'id' : id}
               ...
               if len(features_dict) == 2:
                    # If there are no features, create an empty model
